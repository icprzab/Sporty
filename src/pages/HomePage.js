...
import { collection, getDoc, limit, startAfter, setDoc, updateDoc, collectionGroup, getDocs, deleteDoc, doc, query, where, orderBy, serverTimestamp, onSnapshot } from "firebase/firestore";
...
function Home() {
    ...
    useEffect(() => {
        if (displayComment) {
            ...
            const q = query(commentRef, orderBy("createAt", "desc"), startAfter(lastComment), limit(dataNumber))
            const q2 = query(commentRef, orderBy("createAt", "desc"), startAfter(lastComment), limit(dataNumber + 1))
    ...
    useEffect(() => {
        ...
        if (newPost !== "發文") {
            ...
            if (page > 0) {
                ...
                const q = query(postCollectionRef, where("type", "==", getType), order, startAfter(lastVisible), limit(dataNumber))
                const q2 = query(postCollectionRef, where("type", "==", getType), order, startAfter(lastVisible), limit(dataNumber + 1))
                getDocs(q2)
                    .then(dataNext => {
                        if (dataNext.docs.length <= dataNumber) {
                            getDocs(q)
                                ...
                                <div className={styles.middle_head}>
                                    <div className={styles.head}>
                                        <div className={styles.head_content}>
                                            <div className={styles.head_content_img}><img src={typeIcon} onClick={closeComment}></img></div>
                                            ...
                                            <div className={styles.head_content_text}>
                                                ...
                                                <button className={LatestType ? styles.head_content_text2 : styles.head_content_text3} onClick={LatestPost}>最新貼文</button>
                                            </div>
                                                                    ...
                                                                    <div className={styles.under_left_content_inside}>
                                                                        ...
                                                                        <div className={styles.under_left_text_div}>
                                                                            {showText.includes(post.id) ?
                                                                                <div className={styles.under_left_text}>{post.content}</div> : <div className={styles.under_left_text}>{post.postLess}</div>}
