...
function MyPage() {
  ...
  useEffect(() => {
    function handleClickOutside(event) {
      if (
        refs.current[clickIndex] &&
        !refs.current[clickIndex].contains(event.target)
      ) {
        if (displayButton) {
          closeEdit();
        }
  ...
  function deletePost(e) {
    ...
    deleteDoc(deleteRef);
    const deleteComment = collection(
      db,
      "users",
      e.uid,
      "post",
    ...
    getDocs(deleteComment).then((response) => {
  ...
  useEffect(() => {
    if (EditContent) {
      EditRefs.current[clickIndex].scrollTop =
        EditRefs.current[clickIndex].scrollHeight;
    }
  ...
  useEffect(() => {
    if (displayComment) {
      const dataNumber = 40;
      const commentPostRef = collection(
        db,
        "users",
        commentInfo.uid,
        "post",
  ...
  useEffect(() => {
    ...
    const snapshot = onSnapshot(userDoc, (snapshot) => {
      if (snapshot.data().BirthDay == null) {
        setUsers({ ...snapshot.data(), day: "不能說的秘密" });
      }
      if (snapshot.data().BirthDay !== null) {
        setUsers({ ...snapshot.data(), day: snapshot.data().BirthDay });
      }
    });
    ...
    if (page > 0) {
      if (newPost !== "發文") {
        ...
        if (mytype === "Collect") {
          ...
          const q = query(
            Collect_all,
            where("collectName", "array-contains", getUser),
            orderBy("createAt", "desc"),
            startAfter(lastVisible),
            limit(dataNumber)
          );
          ...
          getDocs(q2).then((dataNext) => {
            if (dataNext.docs.length <= dataNumber) {
              getDocs(q).then((response) => {
                ...
                setPosts([...posts, ...postArray]);
                const EditPostArray = postInfo.filter((EditPostID) =>
                  EditPostID.uid.includes(getUser)
                );
                ...
                <div className={displayComment ? styles.comment_middle : ""}>
                  ...
                  >
                    ...
                    <div className={displayComment ? styles.comment : ""}>
                      <div
                        className={displayComment ? styles.comment_member : ""}
                      >
                        <div
                          className={
                            displayComment
                          ...
                          onClick={() => closeComment("clickAvatar")}
                        >
                          <img
                            className={
                              displayComment
            ...
            <div className={styles.homepage}>
              <div className={styles.homepage_inside}>
                <div className={styles.middle}>
                  <div
                    className={myNavbar ? "middle_head active" : "middle_head"}
                  >
                        ...
                        <div className={styles.all_title}>
                          <div className={styles.title}>
                            <div className={styles.title_content}>
                              <div className={styles.title_content_img}>
                                <img src={url}></img>
                              </div>
                              <div className={styles.title_content_profile}>
                                ...
                                <div
                                  className={styles.title_content_line}
                                ></div>
                                <div className={styles.title_content_intro}>
                                  {users.About}
                                </div>
                              </div>
                              ...
                              <div className={styles.title_content_right}>
                                <div
                                  className={styles.title_content_post}
                                  onClick={mypost}
                                >
                                  <div
                                    className={
                                      mypostType
                                        ? styles.title_content_post_inside
                                        : styles.title_content_post_inside2
                                    }
                                  >
                                    我的
                                  </div>
                                ...
                                <div
                                  className={styles.title_content_collection}
                                  onClick={mycollect}
                                >
                                  <div
                                    className={
                                ...
                                {posts.map((post, index) => (
                                  ...
                                  >
                                    <div
                                      ...
                                      className={
                                        displayButton.includes(post.id)
                                          ? styles.under_left_content_edit_outside
                                          : styles.under_left_content_edit_outside_none
                                      }
                                    >
                                      <button
                                        onClick={() => showEdit(post, index)}
                                      >
                                        編輯
                                      </button>
                                    ...
                                    <div
                                      className={
                                        styles.under_left_content_middle
                                      }
                                    >
                                      <div
                                        className={
                                      ...
                                      >
                                        <div
                                          className={styles.under_left_title}
                                        >
                                          <div
                                          ...
                                          <div
                                            className={styles.under_left_top}
                                          >
                                            <div
                                            ...
                                            <img
                                              className={
                                                EditPosts.includes(post.id)
                                                  ? styles.under_left_top_img
                                                  : styles.under_left_top_img_none
                                              }
                                              src={option}
                                              onClick={() =>
                                                showButton(post.id, index)
                                              }
                                            ></img>
                                          </div>
                                        </div>
                                        <div>
                                          {displayEdit.includes(post.id) ? (
                                            ...
                                            >
                                              <textarea
                                                className={
                                                  styles.under_left_input
                                                }
                                                value={EditContent}
                                                onChange={inputContent}
                                                ref={(el) =>
                                                  (EditRefs.current[index] = el)
                                                }
                                                style={{
                                                  height: "auto",
                                                  minHeight: "100px",
                                            ...
                                            <div
                                              className={
                                                styles.under_left_text_div
                                              }
                                            >
                                              {showText.includes(post.id) ? (
                                                <div
                                                  className={
                                                    styles.under_left_text
                                                  }
                                                >
                                                  {post.content}
                                                </div>
                            ...
                            <div className={styles.under_right}>
                              <div className={styles.under_right_rules}>
                                ...
                                <div className={styles.under_right_rules_info}>
                                  ...
                                  <div
                                    className={
                                      styles.under_right_rules_info_content
                                    }
                                  >
                                    <div
                                      className={
                                        styles.under_right_rules_info_content_left
                                      }
                                    >
                                      性別
                                    </div>
                                    <div
                                      className={
                                        styles.under_right_rules_info_content_right
                                      }
                                    >
                                      {users.Gender}
                                    </div>
