...
function MyPage() {
        ...
        getDocs(CollectionRef)
            .then(response => {
                ...
                for (let i = 0; i <= userInfo.length - 1; i++) {
                    const docRef = doc(db, "users", userInfo[i].postUser, "post", userInfo[i].id)
                    getDoc(docRef)
                        .then(response => {
                            collectionPost.push(response.data())
                            setPosts(collectionPost)
    ...
    useEffect(() => {
        ...
        const q2 = query(postCollectionRef, orderBy("createAt", "desc"))
